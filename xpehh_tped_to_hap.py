...
def main():
    ...
    path = "/home/tx56/palmer_scratch/deepsweep_empirical/1kgp_AA_joint_tped"

    for pop in target_pops:
        for i in range(1, 23):
            input_file = f"{path}/{pop}.{i}.AA.joint.tped"
            
            if not os.path.exists(input_file):
                print(f"Skipping {pop} chromosome {i} as {input_file} does not exist.")
                continue

            os.makedirs("/home/tx56/palmer_scratch/deepsweep_empirical/1kgp_xpehh", exist_ok=True)
            output_file_hap = f"/home/tx56/palmer_scratch/deepsweep_empirical/1kgp_xpehh/{pop}.{i}.joint.hap"
            output_file_map = f"/home/tx56/palmer_scratch/deepsweep_empirical/1kgp_xpehh/{pop}.{i}.joint.map"
