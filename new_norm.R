...
# Function to calculate and normalize delihh
calculate_normalize_delihh <- function(input_file, norm_file, output_file) {
  cat("Processing delihh file:", input_file, "\n")
  
  # Read the input file
  data <- tryCatch({
    read.table(input_file, header = TRUE, sep = "\t", check.names = FALSE)
  }, error = function(e) {
    cat("Error reading input file:", e$message, "\n")
    return(NULL)
  })
  ...
  required_columns <- c("Freq", "iHH_0", "iHH_1")
  if (!all(required_columns %in% colnames(data))) {
    cat("Required columns not found in data. Columns present:", colnames(data), "\n")
    stop("Input file does not have the required columns.")
  }
  ...
  # Calculate delihh
  data$delihh <- as.numeric(data$iHH_1) - as.numeric(data$iHH_0)
  
  # Convert frequencies to numeric
  data$Freq <- as.numeric(as.character(data$Freq))
  if (any(is.na(data$Freq))) {
    cat("NA values found in Freq column after conversion to numeric.\n")
    stop("NA values found in Freq column after conversion to numeric.")
  }
  ...
  # Function to find the closest bin
  find_closest_bin <- function(freq, norm_data) {
    idx <- which.min(abs(norm_data$bin - freq))
    return(norm_data[idx, ])
  }
  
  # Normalize delihh
  data$norm_delihh <- apply(data, 1, function(row) {
    p1 <- as.numeric(row[["Freq"]])
    bin_info <- find_closest_bin(p1, norm_data)
    mean <- bin_info$mean
    std <- bin_info$std
    if (is.na(mean) || is.na(std) || std == 0) {
      return(NA)
    }
    norm_value <- (as.numeric(row[["delihh"]]) - mean) / std
    return(round(norm_value, 4))
  })
...
for (pattern_info in patterns) {
  ...
  for (file in files) {
    ...
    norm_file <- file.path(base_path, "bins", paste0(pop_name, "_", normalize_value_column_name(pattern_info$value_col), "_bin.csv"))
...
# Process delihh separately
pattern_dir <- file.path(base_path, "normed")
files <- list.files(pattern_dir, pattern = "norm_iHS.*\\.tsv$", full.names = TRUE)
...
for (file in files) {
  ...
  norm_file <- file.path(base_path, "bins", paste0(pop_name, "_delihh_bin.csv"))
  output_file <- file.path(output_dir, paste0("norm_delihh_", pop_name, "_", chr_num, ".tsv"))
  ...
  calculate_normalize_delihh(file, norm_file, output_file)
}
